...
void Renderer::addCommand(RenderCommand* command, int renderQueueID)
{
	...
	if (Director::getInstance()->getTotalFrames() == 5)
	{
		if (command->getType() == RenderCommand::Type::TRIANGLES_COMMAND)
		{
			auto cmd = static_cast<TrianglesCommand*>(command);
			CCLOG("[addCommand] nV: %d, nI: %d", cmd->getVertexCount(), cmd->getIndexCount());
			//CCLOG("%d", _queuedTotalIndexCount);
		}
		else
		{
			CCLOG("[addCommand] type: %d", (int)command->getType());
		}
	}
}
...
void Renderer::TriangleCommandBufferManager::createBuffer()
{
	...
	const auto iSize = Renderer::INDEX_VBO_SIZE * sizeof(uint16_t);
    auto tmpData = malloc(iSize);
    if (!tmpData)
        return;
    auto indexBuffer = device->newBuffer(iSize, backend::BufferType::INDEX, backend::BufferUsage::DYNAMIC);
    if (! indexBuffer)
    {
        free(tmpData);
        vertexBuffer->release();
    ...
    indexBuffer->updateData(tmpData, iSize);
    free(tmpData);
